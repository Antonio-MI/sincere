    ...
    with batch_processing_lock:
        if running_request_batches.get(model_alias):
            ...
            if padding:
                ...
                padded_batch_size = get_padded_batch_size(original_batch_size, allowed_batch_sizes)
                if original_batch_size < padded_batch_size:
                    #padding_start_time = time.perf_counter()
                    while len(batch) < padded_batch_size:
            ...
            logging.debug(f"Updated batch size:{updated_batch_size}")
